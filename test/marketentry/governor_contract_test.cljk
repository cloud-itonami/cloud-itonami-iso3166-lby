  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Tripoli Trading & Services Office"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Tripoli Trading & Services Office" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest office-balance-below-floor-is-held-and-unoverridable
  (testing "declared representative-office bank balance (LYD 90,000) below the Ministry of Economy and Trade's documented LYD 150,000 floor -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:office-balance-below-floor} (-> (store/ledger db) last :basis)))
...
(deftest office-balance-exactly-at-floor-is-not-held-by-the-flagship-check
  (testing "eng-1 declares an office balance exactly at the LYD 150,000 floor -- meeting the floor is fine, not a violation"
    (let [[_db actor] (fresh)
          _ (assess! actor "t14pre" "eng-1")
          _ (draft! actor "t14pre" "eng-1")
          res (exec-op actor "t14" {:op :filing/submit :subject "eng-1"} operator)]
