(ns marketentry.marketentryllm
  ...
  the Libya (LBY) company/business-registration market-entry compliance
  actor.
  ...
  cited), never a committed record or a real Ministry of Economy and
  Trade / Commercial Registry filing. Every output is censored
  downstream by `marketentry.governor` before anything touches the SSoT,
  and `:filing/draft`/`:filing/submit` proposals NEVER auto-commit at
    ...
    {:summary    (str subject " 向け提出ドラフト提案"
     ...
     :rationale  (if e
                   (str "jurisdiction=" (:jurisdiction e)
                        " registration-channel=" (:registration-channel e))
                   "engagementが見つかりません")
     :cites      (if e [subject] [])
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world Ministry of Economy and Trade
  / Commercial Registry filing submission."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向け提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "office-balance-lyd=" (:office-balance-lyd e)
